...
const usePoseTracking = ({
  ...
  const calculateHipExtension = useCallback((shoulder, hip, knee) => {
  ...
  const onResults = useCallback((results) => {
    ...
    const knee = [landmarks[kneeIdx].x * width, landmarks[kneeIdx].y * height];

    const newAngle = calculateHipExtension(shoulder, hip, knee);
    setAngle(newAngle);
    ...
    if (newAngle > 15) {
      if (!hasShownPositiveAngleToast.current) {
        toast.error(
          "Posizione scorretta! Stai flettendo l'anca invece di estenderla! Mettiti dritto col busto e spingi l'anca indietro per correggere il movimento.",
          {
            position: "top-center",
            autoClose: 3000,
            draggable: true,
