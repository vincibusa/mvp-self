...
const HipFlexion = () => {
  ...
  const handleExpire = useCallback(() => {
    setIsTracking(false);
    side === "left" ? localStorage.setItem("name", "Report Flessione Anca Sinistra") : localStorage.setItem("name", "Report Flessione Anca Destra");
  ...
  if (!side) {
    return <SideSelectionModal onSelect={setSide} exerciseType="Hip Flexion" />;
  }
  return (
    <div
    ...
    >
      ...
      <div className="absolute bottom-4 left-0 right-0 flex items-center justify-center space-x-4 z-50">
        {isTracking && (
          <Summary
            name={"Massima Flessione"}
            maxFlexion={maxFlexion}
            classifyFlexion={classifyFlexion}
          />
        )}
